...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const login = async (credentials: LoginCredentials): Promise<void> => {
    ...
    try {
      ...
      const isClient =
        credentials.email === 'client@nano.com' && credentials.password === 'nano321';
      const isUser = credentials.email === 'user@nano.com' && credentials.password === 'user123';

      // Employee system credentials
      const isEmployee =
        credentials.email === 'john.doe@nano.com' && credentials.password === 'employee123';
      const isManager =
        credentials.email === 'sarah.manager@nano.com' && credentials.password === 'manager123';
      const isSystemAdmin =
        credentials.email === 'admin.system@nano.com' && credentials.password === 'admin123';
