...
export class EmployeeService {
  ...
  static async authenticateEmployee(
    email: string,
    password: string
  ...
  static async getEmployeeDashboardData(
    employeeId: string
  ): Promise<ApiResponse<EmployeeDashboardData>> {
    try {
      ...
      const currentWeekHours = currentWeekTickets.reduce(
        (sum, ticket) => sum + ticket.totalHours,
        0
      );
      const currentMonthHours = currentMonthTickets.reduce(
        (sum, ticket) => sum + ticket.totalHours,
        0
      );
      const currentWeekEarnings = currentWeekTickets.reduce(
        (sum, ticket) => sum + ticket.totalAmount,
        0
      );
      const currentMonthEarnings = currentMonthTickets.reduce(
        (sum, ticket) => sum + ticket.totalAmount,
        0
      );
      ...
      for (let i = 7; i >= 0; i--) {
        ...
        const weekTickets = employeeTickets.filter(
          (ticket) =>
            ticket.startTime >= weekStart &&
            ticket.startTime <= weekEnd &&
  ...
  static async createWorkTicket(
    employeeId: string,
    ticketData: CreateTicketForm
  ): Promise<ApiResponse<WorkTicket>> {
    try {
  ...
  static async getManagerDashboardData(
    managerId: string
  ): Promise<ApiResponse<ManagerDashboardData>> {
    try {
      ...
      const teamStats: TeamStats = {
        ...
        averageHourlyRate:
          teamMembers.reduce((sum, member) => sum + member.hourlyRate, 0) / teamMembers.length,
        productivityScore: Math.min(
          100,
          (approvedTeamTickets.length / Math.max(teamTickets.length, 1)) * 100
        ),
  ...
  static async getAdminDashboardData(): Promise<ApiResponse<AdminDashboardData>> {
    try {
      ...
      const totalPendingTickets = mockWorkTickets.filter(
        (ticket) => ticket.status === 'pending'
      ).length;
      const approvedTickets = mockWorkTickets.filter((ticket) => ticket.status === 'approved');
      const totalMonthlyPayroll = approvedTickets.reduce(
        (sum, ticket) => sum + ticket.totalAmount,
        0
      );
  ...
  static async exportPayrollData(startDate: Date, endDate: Date): Promise<ApiResponse<string>> {
    try {
      ...
      const ticketsInRange = mockWorkTickets.filter(
        (ticket) =>
          ticket.startTime >= startDate &&
          ticket.startTime <= endDate &&
          ticket.status === 'approved'
      );

      // Generate CSV content
      const csvHeader =
        'Employee ID,Employee Name,Department,Hours,Hourly Rate,Total Amount,Date\n';
      const csvRows = ticketsInRange
        .map((ticket) => {
          const employee = mockEmployees.find((emp) => emp.id === ticket.employeeId);
